...
const LOGIN_URL = 'https://rest-api-69mq.onrender.com/user/login';
...
const REGISTER_URL = 'https://rest-api-69mq.onrender.com/user/signup';
...
export const UserApi = async (userIdd) => {
    const USER_URL = `https://rest-api-69mq.onrender.com/attendance/${userIdd}`;
...
export const AttendanceApi = async (data) => {
  try {
    ...
    const ATTENDANCE_URL = 'https://rest-api-69mq.onrender.com/attendance/';
...
export const CurrentAttendanceByDate = async () => {
  const token = localStorage.getItem('authToken');
  const userId = localStorage.getItem('userId');
  const CURRENT_ATTENDANCE_URL = 'https://rest-api-69mq.onrender.com/admin';
...
export const studentbyRegisterNo = async (registrationNumber) => {
  const USER_URL = `https://rest-api-69mq.onrender.com/admin/${registrationNumber}`;
  const token = localStorage.getItem('authToken');
  return await axios.get(USER_URL, {
    headers: {
      'Authorization': `Bearer ${token}`,
...
export const getDepartmentReport = async (department) => {
  const USER_URL = `https://rest-api-69mq.onrender.com/admin/department/${department}`;
  const token = localStorage.getItem('authToken');
  return await axios.get(USER_URL, {
    headers: {
      'Authorization': `Bearer ${token}`,
...
export const allData = async () => {
  const USER_URL = `https://rest-api-69mq.onrender.com/admin/all`;
  const token = localStorage.getItem('authToken');
  return await axios.get(USER_URL, {
    headers: {
      'Authorization': `Bearer ${token}`,
...
export const searchUserByUserId = async (userId) => {
  const API_URL = 'https://rest-api-69mq.onrender.com/admin/';
  const token = localStorage.getItem('authToken');
...
export const getUserMessages = async () => {
  ...
  try {
    const response = await axios.get(`https://rest-api-69mq.onrender.com/attendance/message/${userId}`, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
      }
