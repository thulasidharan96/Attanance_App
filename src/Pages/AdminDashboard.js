...
const AdminDashboard = () => {
    ...
    const fetchAttendanceData = async () => {
        try {
            const response = await CurrentAttendanceByDate(); // Fetch API data
            ...
            if (Array.isArray(data)) {
                ...
                setAttendanceData([]); // If data is not an array, fallback to empty array
            }
        } catch (error) {
            console.error("Error fetching attendance data:", error);
            setAttendanceData([]); // Fallback to empty array on error
        }
    };
            ...
            <main className="flex-grow p-6">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="flex justify-between items-center mb-8">
                        ...
                        <button onClick={() => logout()} className={`${filterButtonStyle} bg-red-500 hover:bg-red-600`}>
                            Logout
                        </button>
                    </div>
                    ...
                    <div className="flex space-x-4 mb-8">
                        {['attendance', 'reports', 'settings'].map((tab) => (
                            <button
                                ...
                                className={`${tabButtonStyle} ${activeTab === tab ? 'bg-cyan-600 text-white shadow-lg' : 'bg-white text-gray-600 hover:bg-gray-100'}`}
                            >
                    ...
                    {activeTab === 'attendance' && (
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                                {[{ title: 'Total Students', value: totalStudents, color: 'bg-blue-500' },
                                  { title: 'Present Today', value: presentCount, color: 'bg-green-500' },
                                  { title: 'Absent Today', value: absentCount, color: 'bg-red-500' }].map((stat, index) => (
                                    <div key={index} className="bg-white rounded-xl shadow-md p-6 border-t-4 border-b-4 hover:shadow-lg transition-shadow">
                                        <h3 className="text-gray-500 text-sm font-medium">{stat.title}</h3>
                                        <p className={`text-2xl font-bold mt-2 ${stat.color.replace('bg-', 'text-')}`}>{stat.value}</p>
                                    </div>
                                ))}
                            </div>
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {attendanceData.map((item) => (
                                                <tr key={item._id} className="hover:bg-gray-50 transition-colors">
                                                    <td className="px-6 py-4 whitespace-nowrap font-medium">{item.userId}</td>
                                                    <td className="px-6 py-4 whitespace-nowrap">{item._id}</td>
                                                    <td className="px-6 py-4 whitespace-nowrap">{item.department}</td>
                                                    <td className="px-6 py-4 whitespace-nowrap">{item.dateOnly}</td>
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${item.attendanceStatus === 'present' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
